        ...
        isPlayerInQueues(message.author.id)
            .then(queues => {
                if (queues.some(queue => queue.level === rsLevel)) {
                    ...
                    queryDB(`UPDATE rsqueueuser SET lastseenTimestamp=${d} WHERE playerID = ${message.member.id} AND level = ${rsLevel}`)
                }
                else {
        ...
        logrun(level, d).then(() => {
            sendRSEmbed(level, true).then(() => {
                sendQueueStartMessage(level).then(() => {
                    queryDB(`DELETE FROM rsqueueuser WHERE level = ${level}`).catch(err => { })
                }).catch(err => { })
            }).catch(err => { })
        }).catch(err => { })
    }).catch(err => { })
                        ...
                        .then(() => {
                            getCurrentQueue(playersToKick[j].level) //this is to get the length of the queue
                                .then(currentQueue => {
                                    ...
                                    let levelQueued = false
                                    for (let k = 0; k < levelsToUpdate.length; k++) {
                                        if (levelsToUpdate[k] === playersToKick[j].level) {
                                            levelQueued = true
                                        }
                                    }
                                    if (!levelQueued) {
